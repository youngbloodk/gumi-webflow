$(document).ready(function() {
    const sku = getURLParam('product_sku');
    const name = getURLParam('name');
    const email = getURLParam('email');

    if(sku && name && email) {
        $('#postReviewSku').val(sku);
        $('#postReviewFirstName').val(name.split(' ')[0]);
        $('#postReviewLastName').val(name.split(' ')[1]);
        $('#postReviewEmail').val(email);
    }
